...
-- Selecionar TABELA PRODUTO = id, nome e descrição TABELA INFO_PRODUTO = ingredientes
select produto.id_produto,produto.nome,produto.descricao,info_produto.ingredientes 
from produto
inner join info_produto on produto.id_produto=info_produto.id_produto;

-- Selecionar TABELA PEDIDO: id,quantidade,data TABELA CLIENTE: nome,email
select pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome,cliente.email
from pedido
inner join cliente on pedido.id_cliente=cliente.id_cliente;

-- Selecionar TABELA PEDIDO:id,quantidade,data TABELA CLIENTE:nome,email TABELA FUNCIONARIO:nome
select pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome as Nome_Cliente,cliente.email,funcionario.nome as Nome_Funcionario
from pedido
inner join cliente on pedido.id_cliente=cliente.id_cliente
inner join funcionario on pedido.id_funcionario=funcionario.id_funcionario;

-- Selecionar TABELA PEDIDO:id,quantidade,data TABELA CLIENTE:nome,email TABELA FUNCIONARIO:nome TABELA PRODUTO: nome,preco
select pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome as Nome_Cliente,cliente.email,funcionario.nome as Nome_Funcionario,produto.nome as Nome_Produto,produto.preco
from pedido
inner join cliente on pedido.id_cliente=cliente.id_cliente
inner join funcionario on pedido.id_funcionario=funcionario.id_funcionario
inner join produto on produto.id_produto=pedido.id_produto;

-- Selecionar o nome dos clientes com os pedidos com status ‘Pendente’ e 
...
select cliente.nome,count(pedido.id_cliente) as total_de_pedidos
from cliente
inner join pedido on cliente.id_cliente=pedido.id_cliente
group by cliente.nome order by total_de_pedidos desc;

-- Selecionar o preço total (quantidade*preco) de cada pedido
select pedido.id_pedido,sum(pedido.quantidade * produto.preco) as preco_total
from pedido 
inner join produto on pedido.id_produto=produto.id_produto
group by pedido.id_pedido order by preco_total desc;

-- Criação de view entre as tabelas PEDIDO: id,quantidade e data, CLIENTE: nome e email, FUNCIONARIO: nome, PRODUTO: nome e preco
create view resumo_pedido as 
select pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome as Cliente,cliente.email,funcionario.nome as Funcionario,
produto.nome as Produto,produto.preco from pedido
inner join cliente on pedido.id_cliente = cliente.id_cliente
inner join funcionario on pedido.id_funcionario = funcionario.id_funcionario
inner join produto on pedido.id_produto = produto.id_produto
group by pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome,cliente.email,funcionario.nome,
produto.nome,produto.preco;

-- Selecione o id do pedido, nome do cliente e o total (quantidade * preco) de cada pedido da view resumo_pedido
select id_pedido,cliente,sum(quantidade * preco) as total from resumo_pedido group by id_pedido,cliente order by id_pedido asc;

-- Atualiza o view resumo pedido, adicionando campo total
create or replace view resumo_pedido as
select pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome as Cliente,cliente.email,funcionario.nome as Funcionario,
produto.nome as Produto,produto.preco,sum(pedido.quantidade * produto.preco) as total from pedido
inner join cliente on pedido.id_cliente = cliente.id_cliente
inner join funcionario on pedido.id_funcionario = funcionario.id_funcionario
inner join produto on pedido.id_produto = produto.id_produto
group by pedido.id_pedido,pedido.quantidade,pedido.data_pedido,cliente.nome,cliente.email,funcionario.nome,
...
select id_pedido,cliente,total from resumo_pedido group by id_pedido,cliente order by id_pedido asc;
...
begin 
 ...
 select ingredientes into lista_ingredientes from info_produto where id_produto = idProduto;
 return lista_ingredientes;
end //
delimiter ;

-- Execute a função ‘BuscaIngredientesProduto’ com o id de produto 10
select BuscaIngredientesProduto(10) as lista_ingredientes;
...
begin
 ...
 declare valor_pedido decimal(5,2);
 
 select round(avg(total),2) into media from resumo_pedido;
 select total into valor_pedido from resumo_pedido where id_pedido = idPedido;
 
 set resultado = 
 case
	when valor_pedido > media then "Acima da Média"
    when valor_pedido < media then "Abaixo da Média"
    else "Igual a Média"
end;
return resultado;
end //
...
select round(avg(total),2) as media from resumo_pedido;
select id_pedido,total,mediaPedido(id_pedido) from resumo_pedido;
